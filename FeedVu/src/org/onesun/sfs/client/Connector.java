...
public class Connector {
	public AggregatedView view = null;
	
	public Connector(AggregatedView w){
		view = w;
	}
					...
					if(anEvent instanceof MessageEvent){
						...
						try {
							...
							try {
								...
								nodes = messageDocument.getElementsByTagName("entry");
							...
							if(nodes != null && nodes.getLength() > 0){
								for(int index = 0; index < nodes.getLength(); index++){
									...
									DataModel.update(element);

									if(view != null){
										view.addToLeftPanel(element);
										view.updateAll();
									}
								}
							}
