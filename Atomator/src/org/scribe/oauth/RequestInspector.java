...
public class RequestInspector {
	...
	public static String getJsonInfo(OAuthSigner signer, Request request,
			Token accessToken) {
		...
		addField("signature", signer.getSignature(toSign, accessToken
				.getSecret()), info);

		// HttpRequest
		info.append("\"httpRequest\":{");
		addField("verb", request.getMethod().name(), info);
		addField("url", request.getUrl(), info);
		addField("body", request.getBody(), info);
	...
	private static final void addField(String key, String val,
			StringBuilder info) {
		info.append("\"" + key + "\":\"" + val + "\"");
		info.append(",");
	}

	private static final void addObject(String key, String val,
			StringBuilder info) {
		info.append("{");
