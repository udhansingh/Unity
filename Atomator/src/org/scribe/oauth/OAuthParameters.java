...
import org.onesun.utils.encoders.*;
...
class OAuthParameters {
	...
	public String asSortedFormEncodedString() {
		...
		for (String key : sortedKeys())
			buffer.append(URL.percentEncode(key)).append('=').append(
					URL.percentEncode(params.get(key))).append('&');
	...
	public String asOAuthHeader() {
		...
		// Udy
		if(extendedParams != null && extendedParams.size() > 0){
			for(String key : extendedParams.keySet()){
				if(key != null && key.length() > 0){
					buffer.append(key).append('=').append('"').append(URL.percentEncode(extendedParams.get(key))).append("\", ");
				}
			}
		}
		// Udy
		...
		String header = removeLast(buffer, ", ");
		return header;
