...
import org.onesun.utils.encoders.HMAC;
import org.onesun.utils.encoders.URL;
...
class OAuthSigner {
	...
	public void signForRequestToken(Request request, String callback, 
			Map<String, String> extendedParams // Added By Udy
			){
		
		params.put(OAuth.CALLBACK, callback);
	
		// Added by Udy to support outh extensions
		if(extendedParams != null){
			params.setExtendedParams(extendedParams);
			for(String key : extendedParams.keySet()){
				params.put(key, extendedParams.get(key));
			}
		}
		// Udy
	...
	public String getStringToSign(Request request, CallType type){
		String verb = URL.percentEncode(request.getMethod().name());
